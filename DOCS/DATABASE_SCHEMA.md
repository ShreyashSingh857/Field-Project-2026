...
Master list of villages in the system. Each village belongs to a Ward Member's jurisdiction.
...
- `gram_panchayat` can create `ward_member`
- `ward_member` can create `workers`

---

### 4. `workers`

Safai Mitra workers, created and managed by `ward_member`.
...
- `created_by_admin_id` must point to an admin with role `ward_member`.
...
- `assigned_panchayat_id` must reference an admin with role `ward_member`.
...
- `created_task_id` is populated when a `ward_member` converts the issue into a task.
...
| Table                | User App | Worker (Safai Mitra) | Ward Member | Gram Panchayat | Block Samiti | Zilla Parishad |
|----------------------|----------|----------------------|-----------------|----------------|--------------|----------------|
...
workers
    └── created_by_admin_id → admins (ward_member)
